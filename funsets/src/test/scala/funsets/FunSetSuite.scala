...
class FunSetSuite extends FunSuite {
  ...
  test("intersect only contains common elements") {
    new TestSets {
      val s = union(s1, s2)
      val t = union(s2, s3)
      val u = intersect(s, t)
  ...
  test("diff only contains elements not in the other") {
    new TestSets {
      val s = union(s1, s2)
      val t = union(s2, s3)
      val u = diff(s, t)
  ...
  test("filter returns elements of a set that are accepted by a given predicate") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      val t = filter(s, x => x == 1)
