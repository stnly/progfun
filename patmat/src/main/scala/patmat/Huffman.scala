...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.groupBy(identity).map(p => (p._1, p._2.length)).toList.sortBy(_._1)
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: tail => (makeCodeTree(left, right) :: tail).sortBy(weight)
    case _ => trees
  }
   ...
   */
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = text.flatMap(c => encodeC(tree, c, List[Bit]()))

  def encodeC(tree: CodeTree, c: Char, acc: List[Bit]): List[Bit] = tree match {
    case Fork(l, r, _, _) => if (chars(l).contains(c)) encodeC(l, c, acc ::: List(0))
			     else encodeC(r, c, acc ::: List(1))
    case _ => acc
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def inner(current: CodeTree, acc: CodeTable): CodeTable = current match {
      case Fork(l, r, _, _) => mergeCodeTables(inner(l, acc), inner(r, acc))
      case Leaf(c, _) => mergeCodeTables(List((c, encode(tree)(List(c)))), acc)
    }
    inner(tree, List())
  }
